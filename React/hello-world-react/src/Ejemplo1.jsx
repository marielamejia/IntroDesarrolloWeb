import { useState } from 'react'
import reactLogo from './assets/react.svg'
import viteLogo from '/vite.svg'
import './App.css'

export const MiBotonDeMarca = (props) =>{
  return <button style={{backgroundColor: "purple"}}>
    {props.tittle}
    </button>
}
export const MiBotonDeMarca2 = ({title,primario})=>{
  const esPrimario = primario || false; 
  const colorDeFondo = esPrimario? "purple":"grey"; 
  const colorDeTexto = esPrimario? "yellow":"white"; 
  return(
    <button style = {{backgroundColor: colorDeFondo, color:colorDeTexto}}>
      {Profile} 
    </button> 
  ); 
}; 

function App() {
  const [count, setCount] = useState(0)

  const botonUno = {
    title: "Boton 1",
    primario:false, 
  }; 

  return (
    <>
      <div>
        <a href="https://vite.dev" target="_blank">
          <img src={viteLogo} className="logo" alt="Vite logo" />
        </a>
        <a href="https://react.dev" target="_blank">
          <img src={reactLogo} className="logo react" alt="React logo" />
        </a>
      </div>
      <h1>Vite + React</h1>
      <div className="card">
        <button onClick={() => setCount((count) => count + 1)}>
          count is {count}
        </button>
        <p>
          Edit <code>src/App.jsx</code> and save to test HMR
        </p>
      </div>
      <p className="read-the-docs">
        Click on the Vite and React logos to learn more
      </p>
      <div>
        <MiBotonDeMarca tittle="subir" />
        <MiBotonDeMarca tittle="enviar" />
        <MiBotonDeMarca tittle="borrar" />
        <MiBotonDeMarca2 title={"Boton 2"} />
      </div>
    </>
  )
}

export default App